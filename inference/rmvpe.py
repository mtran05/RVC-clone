...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        raise SystemExit("Usage: python -m inference.rmvpe <audio-file> <rmvpe.pt>")
    ...
    rmvpe = RMVPE(sys.argv[2], is_half=False, device=device)
    started = ttime()
    f0 = rmvpe.infer_from_audio(audio, thred=0.03)
    logger.info("%s %.2f", f0.shape, ttime() - started)
